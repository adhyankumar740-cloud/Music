...
def initialize_ptb_application():
    ...
    with initialization_lock:
        ...
        try:
            # FIX: Use asyncio.run inside a function to initialize the application and set the webhook.
            # This is run in a separate thread so it doesn't block the main Flask/SocketIO eventlet loop.
            async def init_ptb_and_set_webhook():
                ...
                # Start the PTB background tasks (like cleanups, etc.)
...
def telegram_webhook():
    if request.method == "POST":
        ...
        try:
            ...
            # FIX: Use eventlet.spawn to run the async PTB function in a non-blocking way.
...
if __name__ == '__main__':
    # Initialize PTB application and set the webhook
    initialize_ptb_application() 
    
    PORT = int(os.environ.get('PORT', 8000))
    
    logger.info(f"Starting server in {ASYNC_MODE} mode on port {PORT}...")
    # socketio.run uses the configured async_mode (eventlet or threading)
