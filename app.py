...
def telegram_webhook():
    if request.method == "POST":
        ...
        # 1. Initialization Check: Wait briefly if application is not ready
        if not application or not bot:
            logger.warning("Bot not fully initialized. Waiting briefly.")
            # Give the initialization thread a chance to complete (max 2 seconds)
            for _ in range(20): 
                eventlet.sleep(0.1) # Sleep in eventlet environment
        ...
        # 2. Process Update
        try:
            # Ensures the correct application and bot instances are used
            update = Update.de_json(request.get_json(force=True), bot) 
            
            # Use eventlet.spawn to run the async PTB function in a non-blocking way.
            eventlet.spawn(application.process_update, update)

            # Return 'ok' immediately to satisfy Telegram's webhook requirement
            return 'ok'
        except Exception as e:
