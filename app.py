...
async def play_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Ensure the URL is correctly constructed with HTTPS if deployed securely
...
def initialize_ptb_application():
    ...
    with initialization_lock:
        ...
        try:
            # Webhook set करने के लिए async function को एक अलग thread में चलाएँ
            ...
            # Threading का उपयोग करें क्योंकि eventlet और asyncio.run() एक साथ ठीक से काम नहीं करते हैं
            thread = threading.Thread(target=asyncio.run, args=(init_ptb_and_set_webhook(),))
            thread.start()
            
            logger.info("PTB Application initialization initiated successfully.")
        except Exception as e:
            logger.critical(f"FATAL ERROR during PTB initialization/Webhook set: {e}", exc_info=True)
            application = None; bot = None

# ------------------------------------------------------------------
# --- GLOBAL CORS FIX (Handles Fetch/XHR errors across all endpoints) ---
# ------------------------------------------------------------------
...
def after_request(response):
    ...
    when accessing API endpoints from the Telegram Web App.
    """
...
# ------------------------------------------------------------------
# --- WEBHOOK ENDPOINT (FIXED: using eventlet.spawn) ---
# ------------------------------------------------------------------
...
def telegram_webhook():
    if request.method == "POST":
        ...
        try:
            ...
            # FIX: Coroutine warning और event loop closure को रोकने के लिए eventlet.spawn का उपयोग करें।
            # यह PTB के async call को eventlet के green thread में चलाता है।
            eventlet.spawn(application.process_update, update)
...
# ------------------------------------------------------------------
# --- YOUTUBE DATA API SEARCH ENDPOINT (FIXED: Using httpx) ---
# ------------------------------------------------------------------
