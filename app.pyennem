...
# NOTE: This function is defined as synchronous (def) to be handled by Flask's main thread,
# and it uses asyncio.run() internally to safely execute the Telegram API call.
...
def set_webhook():
    ...
    try:
        ...
        async def run_set_webhook():
            # bot.set_webhook is an async method that must be awaited
            return await bot.set_webhook(url=webhook_url)

        # asyncio.run() is the key fix to resolve the 'Event loop is closed' error 
        # by running the async method in a temporary, fresh event loop.
        ...
        # Return 500 status on critical runtime error
